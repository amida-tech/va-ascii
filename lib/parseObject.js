...
module.exports = [{
    ...
    model: [{
        ...
        process: function (processed, match) {
            var obj = processed;
            if (obj.medications) {
                obj.medications.push({
                    medication_name: match[1]
                });
            } else {
                obj.medications = [{
                    medication_name: match[1]
    ...
    model: [{
        ...
        process: function (processed, match) {
            ...
            if (obj.immunizations) {
                obj.immunizations.push({
                    immunization_name: match[1].trim()
                });
            } else {
                obj.immunizations = [{
                    immunization_name: match[1].trim()
        ...
        process: function (processed, match) {
            var obj = processed;
            if (obj.immunizations) {
                obj.immunizations[obj.immunizations.length - 1][match[1].replace(/ /g, '_').toLowerCase()] = match[2].trim();
            } else {
                if (obj.summary) {
                    obj.summary.push({
                        immunization: match[1].trim(),
                        date: match[2].trim()
                    });
                } else {
                    obj.summary = [{
            ...
            if (obj.newtest) {
                if (obj.labs) {
                    if (obj.labs[obj.labs.length - 1].tests) {
                        obj.labs[obj.labs.length - 1].tests.push({
                            test_name: match[1].trim()
                        });
                    } else {
                        obj.labs[obj.labs.length - 1].tests = [{
                            test_name: match[1].trim()
                        }];
                    }
                }
