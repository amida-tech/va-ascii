...
module.exports = [{
    ...
    model: [{
        ...
        process: function (processed, match) {
            var obj = processed;
            if (obj.medications) {
                obj.medications.push({medication_category: match[1]});
            } else {
                obj.medications = [{medication_category: match[1]}];
            }
            return obj;
        },
        raw: [],
        ...
        pattern: /^(?:Start Date: *)(.*)(?:Stop Date: *)(.*)$/,
        multiple: true,
        model: [],
        process: function (processed, match) {
            var obj = processed;
            if (obj.medications) {
                obj.medications[obj.medications.length - 1].start_date = match[1].trim();
                obj.medications[obj.medications.length - 1].stop_date = match[2].trim();
            }
            return obj;
        },
        raw: [],
        ...
        pattern: /^(?:Pharmacy Name: *)(.*)(?:Pharmacy Phone: *)(.*)$/,
        multiple: true,
        model: [],
        process: function (processed, match) {
            var obj = processed;
            if (obj.medications) {
                obj.medications[obj.medications.length - 1].pharmacy_name = match[1].trim();
                obj.medications[obj.medications.length - 1].pharmacy_phone = match[2].trim();
            }
            return obj;
        },
        raw: [],
    ...
    model: [{
        ...
        process: function (processed, match) {
            ...
            if (obj.newnote) {
                if (obj.notes) {
                    obj.notes.push({date_time: match[1].trim()})
                } else {
                    obj.notes = [{date_time: match[1].trim()}];
                }
            }
            obj.newnote = false;
            return obj;
        ...
        process: function (processed, match) {
            var obj = processed;
            if (obj.notes) {
                if (!obj.notes[obj.notes.length-1].raw_note) {
                    obj.notes[obj.notes.length-1].raw_note = [];
                }
            }
            return obj;
        },
        ...
        process: function (processed, match) {
            ...
            if (obj.notes) {
                if (obj.notes[obj.notes.length-1].raw_note) {
                    obj.notes[obj.notes.length-1].raw_note.push(match[1]+ ': '+match[2].trim());
                } else {
                    obj.notes[obj.notes.length-1][key] = value;
                }
            } else {
                obj[key] = value;
            }
        ...
        process: function (processed, match) {
            ...
            if (obj.notes) {
                if (obj.notes[obj.notes.length-1].raw_note) {
                    obj.notes[obj.notes.length-1].raw_note.push(match[1].trim());
                }
            }
            return obj;
        },
    ...
    model: [{
        ...
        name: 'summary',
        pattern: /(.*)(?: {2,})([01][0-9] .* [12][0-9][0-9][0-9] @ [0-2][0-9][0-5][0-9])/,
        multiple: true,
        model: [],
        process: function (processed, match) {
            var obj = processed;
    ...
    model: [{
        ...
        process: function (processed, match) {
            ...
            if (obj.lastkey) {
                if (obj.immunizations) {
                    if (Object.keys(obj.immunizations[obj.immunizations.length - 1]).indexOf(obj.lastkey) > -1) {
                        obj.immunizations[obj.immunizations.length - 1][obj.lastkey] += ' ' + match[1].trim();
                    }
                }
            }
            return obj;
    ...
    model: [{
        ...
        process: function (processed, match) {
            ...
            if (obj.newlab) {
                if (obj.labs) {
                    obj.labs.push({lab_test: match[1].trim()})
                } else {
                    obj.labs = [{lab_test: match[1].trim()}];
                }
            }
            obj.newlab = false;
            return obj;
        ...
        process: function (processed, match) {
            ...
            if (obj.newtest) {
                if (obj.labs) {
                    if (obj.labs[obj.labs.length - 1].tests) {
                        obj.labs[obj.labs.length - 1].tests.push({test_name: match[1].trim()})
                    } else {
                        obj.labs[obj.labs.length - 1].tests = [{test_name: match[1].trim()}];
                    }
                }
            }
            obj.newtest = false;
        ...
        process: function (processed, match) {
            ...
            var value = match[2].trim();
            if (obj.labs) {
                if (obj.newtest || obj.labs[obj.labs.length - 1].raw_lab) {
                    if (obj.labs[obj.labs.length - 1].raw_lab) {
                        obj.labs[obj.labs.length - 1].raw_lab.push(match[1] + ': ' + match[2].trim());
                    } else {
                        obj.labs[obj.labs.length - 1].raw_lab = [match[1] + ': ' + match[2].trim()];
                    }
                } else {
                    if (obj.labs[obj.labs.length - 1].tests) {
                        obj.labs[obj.labs.length - 1].tests[obj.labs[obj.labs.length - 1].tests.length - 1][key] = value;
                    } else {
                        obj.labs[obj.labs.length - 1][key] = value;
                    }
                }
            } else {
                obj[key] = value;
        ...
        process: function (processed, match) {
            ...
            if (obj.labs) {
                if (obj.labs[obj.labs.length - 1].raw_lab) {
                    obj.labs[obj.labs.length - 1].raw_lab.push(match[1].trim());
                } else {
                    if (obj.labs[obj.labs.length - 1][obj.lastkey]) {
                        obj.labs[obj.labs.length - 1][obj.lastkey] += ' ' + match[1].trim();
                    } else {
                        if (obj.labs[obj.labs.length - 1].tests) {
                            if (obj.labs[obj.labs.length - 1].tests[obj.labs[obj.labs.length - 1].tests.length - 1][obj.lastkey]) {
                                obj.labs[obj.labs.length - 1].tests[obj.labs[obj.labs.length - 1].tests.length - 1][obj.lastkey] += ' ' + match[1].trim();
                            }
                        } else {
                            if (obj[obj.lastkey]) {
                                obj[obj.lastkey] += ' ' + match[1].trim();
                            }
                        }
                    }
                }
    ...
    model: [{
        ...
        process: function (processed, match) {
            var obj = processed;
            if (obj.tests) {
                obj.tests.push({medication_name: match[1]});
            } else {
                obj.tests = [{medication_name: match[1]}];
            }
            return obj;
        },
        raw: [],
        ...
        process: function (processed, match) {
            var obj = processed;
            if (obj.tests) {
                obj.tests[obj.tests.length - 1].results = [match[1].trim()];
            }
            return obj;
        },
        raw: [],
        ...
        process: function (processed, match) {
            ...
            var key = match[1].replace(/ /g, '_').toLowerCase();
            if (obj.tests) {
                obj.tests[obj.tests.length - 1][key] = match[2].trim();
            } else {
                obj[key] = match[2].trim();
            }
            return obj;
        ...
        process: function (processed, match) {
            var obj = processed;
            if (obj.tests) {
                if (obj.tests[obj.tests.length - 1].results) {
                    obj.tests[obj.tests.length - 1].results.push(match[1].trim());
                }
            }
            return obj;
        },
    ...
    model: [{
        ...
        process: function (processed, match) {
            var obj = processed;
            if (obj.vitals) {
                obj.vitals.push({vital_sign: match[1].trim()});
            } else {
                obj.vitals = [{vital_sign: match[1].trim()}];
            }
            return obj;
        },
        raw: [],
        ...
        name: 'summary',
        pattern: /(.*)(?: {2,})(\S.*)(?: {2,})([01][0-9] .* [12][0-9][0-9][0-9] @ [0-2][0-9][0-5][0-9])/,
        multiple: true,
        model: [],
        process: function (processed, match) {
            var obj = processed;
            if (obj.summary) {
                obj.summary.push({vital_sign: match[1].trim(), reading: match[2].trim(), date:match[3].trim()});
            } else {
                obj.summary = [{vital_sign: match[1].trim(), reading: match[2].trim(), date:match[3].trim()}];
            }
            return obj;
        },
        raw: [],
        ...
        process: function (processed, match) {
            ...
            if (obj.vitals) {
                obj.vitals[obj.vitals.length-1][key] = value;
            } else {
                obj[key] = value;
            }
            return obj;
    ...
    model: [{
        name: 'vitalsname',
        pattern: /^(?:Measurement Type: *)(.*)$/,
        multiple: true,
        model: [],
        process: function (processed, match) {
            var obj = processed;
            if (obj.vitals) {
                obj.vitals.push({measurement_type: match[1]});
            } else {
                obj.vitals = [{measurement_type: match[1]}];
            }
            return obj;
        },
        raw: [],
        ...
        process: function (processed, match) {
            var obj = processed;
            if (obj.vitals) {
                obj.vitals[obj.vitals.length - 1].comments = [match[1].trim()];
            }
            return obj;
        },
        raw: [],
        ...
        process: function (processed, match) {
            ...
            var key = match[1].replace(/ /g, '_').toLowerCase();
            if (obj.vitals) {
                obj.vitals[obj.vitals.length - 1][key] = match[2].trim();
            } else {
                obj[key] = match[2].trim();
            }
            return obj;
        ...
        process: function (processed, match) {
            var obj = processed;
            if (obj.vitals) {
                if (obj.vitals[obj.vitals.length - 1].comments) {
                    obj.vitals[obj.vitals.length - 1].comments.push(match[1].trim());
                }
            }
            return obj;
        },
