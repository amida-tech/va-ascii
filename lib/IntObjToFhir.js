...
IntObjToFhirStream.prototype._transform = function (va, encoding, cb) {
    var outputObj = fhirObject;
    var fhirKeys = Object.keys(fhirObject.Proc);
    async.eachSeries(fhirKeys, function (fhirRow, cb1) {
        async.series([function (cb2) {
            if (outputObj.Proc[fhirRow].self_demographics) {
                outputObj.Proc[fhirRow] = outputObj.Proc[fhirRow].self_demographics(outputObj, va.self_demographics);
                cb2();
            } else {
                cb2();
            }
        }, function (cb2) {
            if (outputObj.Proc[fhirRow].va_demographics) {
                outputObj.Proc[fhirRow] = outputObj.Proc[fhirRow].va_demographics(outputObj, va.va_demographics);
                cb2();
            } else {
                cb2();
            }
        }, function (cb2) {
            if (outputObj.Proc[fhirRow].va_appointments) {
                outputObj.Proc[fhirRow] = outputObj.Proc[fhirRow].va_appointments(outputObj, va.va_appointments);
                cb2();
            } else {
                cb2();
            }
        }, function (cb2) {
            if (outputObj.Proc[fhirRow].self_allergies) {
                outputObj.Proc[fhirRow] = outputObj.Proc[fhirRow].self_allergies(outputObj, va.self_allergies);
                cb2();
            } else {
                cb2();
            }
        }, function (cb2) {
            if (outputObj.Proc[fhirRow].va_allergies) {
                outputObj.Proc[fhirRow] = outputObj.Proc[fhirRow].va_allergies(outputObj, va.va_allergies);
                cb2();
            } else {
                cb2();
            }
        }, function (cb2) {
            if (outputObj.Proc[fhirRow].va_medications) {
                outputObj.Proc[fhirRow] = outputObj.Proc[fhirRow].va_medications(outputObj, va.va_medications);
                cb2();
            } else {
                cb2();
            }
        }, function (cb2) {
            if (outputObj.Proc[fhirRow].self_medications) {
                outputObj.Proc[fhirRow] = outputObj.Proc[fhirRow].self_medications(outputObj, va.self_medications);
                cb2();
            } else {
                cb2();
            }
        }, function (cb2) {
            if (outputObj.Proc[fhirRow].va_immunizations) {
                outputObj.Proc[fhirRow] = outputObj.Proc[fhirRow].va_immunizations(outputObj, va.va_immunizations);
                cb2();
            } else {
                cb2();
            }
        }, function (cb2) {
            if (outputObj.Proc[fhirRow].self_immunizations) {
                outputObj.Proc[fhirRow] = outputObj.Proc[fhirRow].self_immunizations(outputObj, va.self_immunizations);
                cb2();
            } else {
                cb2();
            }
        }, function (cb2) {
            if (outputObj.Proc[fhirRow].va_problems) {
                outputObj.Proc[fhirRow] = outputObj.Proc[fhirRow].va_problems(outputObj, va.va_problems);
                cb2();
            } else {
                cb2();
            }
        }, function (cb2) {
            if (outputObj.Proc[fhirRow].va_admission_discharge) {
                outputObj.Proc[fhirRow] = outputObj.Proc[fhirRow].va_admission_discharge(outputObj, va.va_admission_discharge);
                cb2();
            } else {
                cb2();
            }
        }, function (cb2) {
            if (outputObj.Proc[fhirRow].va_vitals) {
                outputObj.Proc[fhirRow] = outputObj.Proc[fhirRow].va_vitals(outputObj, va.va_vitals);
                cb2();
            } else {
                cb2();
            }
        }, function (cb2) {
            if (outputObj.Proc[fhirRow].self_vitals) {
                outputObj.Proc[fhirRow] = outputObj.Proc[fhirRow].self_vitals(outputObj, va.self_vitals);
                cb2();
            } else {
                cb2();
            }
    ...
    /*
     ...
     _.reduce(va, function (memo, value, index, list) {
...
Factory.prototype.claims = function (va, patient, claimNumber, source, bundle) {
    var self = this;
    var result = [];
    ...
    paymentReconciliation = _.find(bundle.entry, function (val) {
        return val.resource.resourceType === 'PaymentReconciliation' && val.resource.identifier.value === claimNumber;
    });
    ...
    if (!paymentReconciliation.resource.organization && source && source !== '') {
        ...
        org = _.find(bundle.entry, function (val) {
            return val.resource.resourceType === 'Organization' && val.resource.identifier && val.resource.identifier.value === source;
        });
        if (!org) {
            org = {
                "resource": {
    ...
    _.forOwn(va, function (value, key, object) {
...
Factory.prototype.pansInsuranceSubsidy = function (va, patient, empty, source) {
...
Factory.prototype.pharmacies = function (va, patient) {
    var self = this;
    var result = [];
...
Factory.prototype.providers = function (va, patient) {
    var self = this;
    var result = [];
...
Factory.prototype.preventiveServices = function (va, patient, empty, source) {
    var self = this;
    var result = [];
...
Factory.prototype.drugs = function (va, patient, empty, source) {
    var self = this;
    var result = [];
...
Factory.prototype.familyMedicalHistory = function (va, patient) {
    var self = this;
    var result = [];
 ...
 * @param va - input stream
 * @param patient - patient record in FHIR format
 */
Factory.prototype.selfReportedVitalStatistics = function (va, patient) {
 ...
 * @param patient - patient record in FHIR format
 */
Factory.prototype.selfReportedLabsAndTests = function (va, patient) {
 ...
 * @param patient - patient record in FHIR format
 */
Factory.prototype.selfReportedImmunizations = function (va, patient) {
 ...
 */
Factory.prototype.selfReportedImplantableDevice = function (va, patient) {
...
Factory.prototype.selfReportedAllergies = function (va, patient) {
...
Factory.prototype.selfReportedMedicalConditions = function (va, patient) {
...
Factory.prototype.emergencyContact = function (va, patient) {
...
Factory.prototype.demographic = function (va, empty1, empty2, source) {
