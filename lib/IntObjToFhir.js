...
IntObjToFhirStream.prototype._transform = function (va, encoding, cb) {
    var outputObj = {};
    console.log(fhirObject.length);
    async.eachSeries(fhirObject,function(fhirRow, cb1) {
        console.log(fhirRow.path);
        if (fhirRow.path.split('[0]').length > 1) {
            console.log("row has at least one array");
        } else {
            if (_.has(va,fhirRow.path)) {
                console.log("Int Obj has row");
                //use a set to put this in the output Object.
            } else {
                console.log("FHIR Row not found in Int Obj");
            }
        }
        cb1();
    },function(){
